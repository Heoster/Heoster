
# 👋 Hi, I'm Heoster

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=36BCF7&height=180&section=header&text=Heoster&fontSize=70&fontAlignY=35&desc=Harsh%20Saini%20%7C%20Full-Stack%20Engineer%20%7C%20Founder%20of%20CODEEX-AI&descAlignY=55&descSize=16&animation=fadeIn" width="100%"/>
</div>

<div align="center">

**Full-Stack Engineer | Founder of CODEEX-AI | Building the Future with AI & Web Technologies**

[![GitHub Followers](https://img.shields.io/github/followers/Heoster?style=social)](https://github.com/Heoster)
[![Twitter Follow](https://img.shields.io/twitter/follow/The_Heoster_?style=social)](https://twitter.com/The_Heoster_)
[![Profile Views](https://komarev.com/ghpvc/?username=Heoster&color=36BCF7&style=flat-square&label=Profile+Views)](https://github.com/Heoster)

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=22&pause=1000&color=36BCF7&center=true&vCenter=true&width=600&lines=Hi%2C+I'm+Harsh+Saini+(Heoster);Full-Stack+Engineer+%26+AI+Builder;Founder+of+CODEEX-AI;16+yo+from+Khatauli%2C+India;Building+open+AI+for+everyone;React+%7C+Next.js+%7C+TypeScript+%7C+LLMs;3D+Web+Enthusiast" alt="Typing SVG" />

</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 🚀 About Me

I'm **Harsh Saini**, professionally known as **Heoster** — a passionate **Full-Stack Engineer**, student, and founder from **Khatauli (Muzaffarnagar, Uttar Pradesh), India**.

I specialize in building innovative AI-powered applications and seamless web experiences. I love combining cutting-edge AI technology with beautiful, responsive design to create solutions that make a real impact.

Currently studying **Class 12 PCM** at **Maples Academy Khatauli**, and founder of **[CODEEX-AI](https://codeex-ai.vercel.app)**. Driven by the belief that advanced AI tools should be accessible to everyone at **zero cost**, I focus on building open-source platforms, intelligent auto-routers, and full-stack developer applications that serve users worldwide.

Always learning. Always building. 🔥

> *Intelligence should be free, fast, and useful for everyone.*

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 💼 What I Do

<table>
<tr>
<td width="50%" valign="top">

### 🤖 AI & Machine Learning
- **AI Platform Development** — multi-provider AI orchestration
- **AI Integrations** — Groq, OpenAI, Gemini, Hugging Face, Claude, Cerebras, DeepSeek, OpenRouter
- **Large Language Models** — intelligent applications with LLMs
- **Smart Routing** — provider-agnostic intent detection
- **Voice** — Groq Orpheus TTS, Whisper STT
- **Vision & Image Gen** — FLUX, Cloudflare Workers AI
- **AI Education** — making AI accessible & understandable
- LangChain, prompt engineering, LLM integrations

</td>
<td width="50%" valign="top">

### 💻 Full-Stack Web Development
- **Frontend:** React 18, Next.js 14+, TypeScript, Vite, TailwindCSS
- **Backend:** Node.js, Express, Firebase, MongoDB, PostgreSQL
- **Full-Stack:** end-to-end apps with modern practices
- **3D Web:** Three.js, WebGL, interactive experiences
- **PWA:** progressive web apps with native features
- **Deploy:** Vercel & Netlify
- HTML/CSS, API integration, GitHub workflows

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🎨 Design & Animation
- **Interactive 3D Graphics** — custom animations & visual effects
- **Motion Design** — smooth animations with GSAP
- **Responsive UI/UX** — pixel-perfect across all devices
- **Performance First** — optimized for speed & accessibility

</td>
<td width="50%" valign="top">

### 🛠️ Backend, Storage & Tools
- LocalStorage-first architectures
- MongoDB, PostgreSQL, cloud storage
- Git, Docker, Linux, VS Code, Codeex
- Auth: JWT, OAuth, session management
- SEO: JSON-LD, OpenGraph, sitemaps

</td>
</tr>
</table>

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 🛠️ Tech Skills

```text
Languages:      JavaScript, TypeScript, Python, HTML/CSS
Frontend:       React 18, Next.js 14+, Vite, TailwindCSS, GSAP, Three.js
Backend:        Node.js, Express, Firebase, MongoDB, PostgreSQL
AI / ML:        LangChain, OpenAI, Groq, Gemini, Claude, Hugging Face,
                Cerebras, DeepSeek, OpenRouter, Whisper, FLUX
Tools:          Git, Docker, Linux, VS Code, Vercel, Netlify, Codex
```

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=js,ts,react,nextjs,vite,tailwind,nodejs,express,mongodb,postgres,firebase,python,docker,git,linux,vscode,vercel,threejs&theme=dark" />
  </a>
</p>

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 🌟 Featured Projects

| Project | Description | Links |
|:---|:---|:---|
| **[MY Portfolio](https://heoster.vercel.app)** | Interactive 3D portfolio with GSAP animations and dynamic web graphics | [Live](https://heoster.vercel.app) |
| **[SOHAM](https://soham-ai.vercel.app)** | Free AI platform aggregating **35+ models** across top providers. Intent-detecting auto-router classifying queries into 13 types, live web search, PDF analysis, and voice synthesis | [Live](https://soham-ai.vercel.app) |
| **[CODEEX-AI](https://codeex-ai.vercel.app)** | Conversational intelligence platform — smart AI routing, visual problem solving, slash commands, multi-chat management | [Live](https://codeex-ai.vercel.app) |
| **[CloudStore](https://cloudstores.netlify.app)** | Full-featured cloud storage web app to manage, organize, and preview files, scripts, and documents online | [Live](https://cloudstores.netlify.app) |
| **[Codeex-v5](https://github.com/Heoster/codeex-v5)** | Desktop GUI for Ollama — auto-setup, real-time streaming, local chat storage, voice synthesis, 20+ open-source AI models | [GitHub](https://github.com/Heoster/codeex-v5) |
| **[Codeex-v3](https://github.com/Heoster/codeex-v3)** | AI assistant with automatic provider fallbacks, web search, mobile-first design built with Next.js 14 | [GitHub](https://github.com/Heoster/codeex-v3) |
| **[Codeex-focus](https://github.com/Heoster/codeex-focus)** | Focus productivity web app — personalized timetables in five minutes | [GitHub](https://github.com/Heoster/codeex-focus) |
| **[Jarvis](https://github.com/Heoster/jarvis)** | Personal AI assistant powered by Transformers, LangChain, and real-time web capabilities | [GitHub](https://github.com/Heoster/jarvis) |
| **[Sri Shakumbhari Devi Mandir Portal](https://mandir-portal.vercel.app)** | Community donation and tracking portal for a temple in Rampur, Khatauli — verified donor receipts and WhatsApp sharing | [Live](https://mandir-portal.vercel.app) |
| **[Codeex Studio / ClassBoard](https://codeex-studio.vercel.app)** | Interactive digital whiteboarding and teaching tool for classroom use and presentations | [Live](https://codeex-studio.vercel.app) |

### Highlights
- **SOHAM AI Platform** — multi-provider AI orchestration with 26–35+ models
- **CODEEX-AI Solutions** — custom AI integrations & smart applications
- **Personal Portfolio** — interactive 3D + GSAP

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 🎯 Services & Expertise

✅ **AI Platform Development** — custom AI solutions & integrations  
✅ **Full-Stack Web Applications** — modern, scalable solutions  
✅ **PWA Development** — progressive web apps with native features  
✅ **3D Web Experiences** — interactive & immersive interfaces  
✅ **Performance Optimization** — speed & SEO improvements  
✅ **Technical Consulting** — guidance on architecture & best practices  
✅ **Authentication & Security** — JWT, OAuth, session flows, validation schemas  

### How I Use AI in Coding, SEO, Authentication & Full-Stack Development

- **Full-Stack Development:** specialized models (DeepSeek R1, Claude) for boilerplates, clean React/Next.js components, TypeScript refactors, and rapid API debugging
- **Authentication & Security:** audit auth flows (JWT, OAuth, sessions), verify authorization middleware edge cases, generate secure validation schemas
- **SEO Optimization:** structured JSON-LD, optimized metadata (title tags, dynamic OpenGraph cards), semantic HTML, automated sitemaps
- **Development Workflow:** automated refactoring, inline docs, unit tests, database/storage access optimization

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 📊 GitHub Analytics

<div align="center">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=Heoster&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="GitHub Stats" />
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Heoster&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" alt="Top Languages" />
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Heoster&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Heoster&theme=tokyo-night&hide_border=true&area=true" width="100%" alt="Activity Graph" />
</div>

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Heoster&theme=tokyonight&no-frame=true&column=7&margin-w=15" alt="Trophies" />
</div>

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

## 📫 Let's Connect

I'm always open to exciting opportunities, collaborations, and discussions about **AI, web development, and tech innovation**.

<div align="center">

[![Website](https://img.shields.io/badge/Website-heoster.vercel.app-36BCF7?style=for-the-badge&logo=vercel&logoColor=white)](https://heoster.vercel.app)
[![CODEEX-AI](https://img.shields.io/badge/CODEEX--AI-Platform-8B5CF6?style=for-the-badge)](https://codeex-ai.vercel.app)
[![SOHAM](https://img.shields.io/badge/SOHAM-AI-06B6D4?style=for-the-badge)](https://soham-ai.vercel.app)

[![Twitter](https://img.shields.io/badge/X-@The_Heoster_-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/The_Heoster_)
[![Instagram](https://img.shields.io/badge/Instagram-@codeex._.heoster-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/codeex._.heoster)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Harsh%20Heoster-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/harsh-heoster)
[![GitHub](https://img.shields.io/badge/GitHub-Heoster-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Heoster)
[![Email](https://img.shields.io/badge/Email-codeex@email.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:codeex@email.com)

</div>

### Online Profiles & Socials

- **GitHub:** [github.com/Heoster](https://github.com/Heoster)
- **X (Twitter):** [@The_Heoster_](https://x.com/The_Heoster_)
- **Instagram:** [@codeex._.heoster](https://www.instagram.com/codeex._.heoster)
- **LinkedIn:** [linkedin.com/in/harsh-heoster](https://linkedin.com/in/harsh-heoster)
- **Portfolio:** [heoster.vercel.app](https://heoster.vercel.app)
- **CODEEX-AI:** [codeex-ai.vercel.app](https://codeex-ai.vercel.app)
- **Email:** [codeex@email.com](mailto:codeex@email.com)

<img src="https://user-images.githubusercontent.com/73097560/115834477-db03d680-a447-11eb-908a-139a6edaec5c.gif" width="100%">

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=36BCF7&height=120&section=footer"/>
</div>

<div align="center">

**Built with ❤️ in Khatauli, India**  
*Harsh Saini · Heoster · Founder, CODEEX-AI*  
*Last updated: April 2026*

⭐ From [Heoster](https://github.com/Heoster)

</div>
